...
export class SignUpComponent implements OnInit {
  ...
  constructor(private authService: AuthService, private router: Router, private formBuilder: FormBuilder, private snackBar: MatSnackBar) {}
   ...
   ngOnInit() {
    this.registerForm = this.formBuilder.group({
      ...
      mobileno: ['', [Validators.required, Validators.minLength(10), Validators.pattern(/^((\+){1}91){1}[1-9]{1}[0-9]{9}$/)]]
    });
  }
  ...
  onSignup() {
    ...
    if (this.registerForm.value.password === this.registerForm.value.cpassword) {
       ...
       this.authService.createUser( this.registerForm.value.name, this.registerForm.value.email, this.registerForm.value.password, this.registerForm.value.mobileno);
